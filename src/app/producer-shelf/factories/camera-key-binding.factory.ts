...
export class CameraKeyBindingFactory {
    ...
    public createCameraKeyBindings(numberOfCameras: number, rundownId: string): KeyBinding[] {
        return Array(numberOfCameras).fill(null).flatMap((_, index) => [
            this.createInsertCameraOnAirKeyBindings(index + 1, rundownId),
            this.createInsertCameraAsNextKeyBindings(index + 1, rundownId)
        ])
    }
