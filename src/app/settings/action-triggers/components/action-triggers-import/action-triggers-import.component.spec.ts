...
async function configureTestBed(): Promise<ActionTriggersImportComponent> {
  ...
  const mockedActionTriggerParser: ActionTriggerValidator = instance(createMockOfActionTriggerParserService())
  ...
  await TestBed.configureTestingModule({
    providers: [
      ...
      { provide: ActionTriggerValidator, useValue: mockedActionTriggerParser },
...
function createMockOfActionTriggerParserService(): ActionTriggerValidator {
  const mockedActionTriggerParser = mock<ActionTriggerValidator>()
  const mockedSubscription = mock<Subscription>()
  const mockedObservable = mock<Observable<void>>()
  when(mockedObservable.subscribe(anything)).thenReturn(instance(mockedSubscription))
